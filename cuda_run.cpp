...
int main(int argc, char** argv) {
    ...
    upcxx::global_ptr<double, upcxx::memory_kind::cuda_device> gpu_array = gpu_alloc.allocate<double>(1000);
    upcxx::global_ptr<double, upcxx::memory_kind::cuda_device> gpu_to_copy = gpu_alloc.allocate<double>(1000);

    upcxx::global_ptr<double> host_array1 = upcxx::new_array<double>(10);
    upcxx::global_ptr<double> host_array2 = upcxx::new_array<double>(1000);
    double *h1 = host_array1.local();
    double *h2 = host_array2.local();

    upcxx::global_ptr<double, upcxx::memory_kind::cuda_device> gpu_adj = gpu_array+0;
    if (upcxx::rank_me() == upcxx::rank_n()-1){
    ...
    for (int i =0; i < 1000; i+=10){
      ...
      upcxx::copy(host_array1, gpu_adj,10).wait();
      gpu_adj += 10;

    }

    copy_wrapper(gpu_alloc.local(gpu_to_copy),gpu_alloc.local(gpu_array), 1000);

    upcxx::copy(gpu_to_copy, host_array2, 1000).wait();
    ...
    gpu_alloc.deallocate(gpu_to_copy);
